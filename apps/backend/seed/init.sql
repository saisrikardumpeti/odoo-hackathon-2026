...
-- =====================================================================
-- Seed: one bootstrap Admin (nobody can self-elevate via the app,
-- so the first Admin must be inserted directly at deploy time)
-- =====================================================================
INSERT INTO employees (id, name, email, role, status)
VALUES (gen_random_uuid(), 'System Admin', 'admin@assetflow.local', 'Admin', 'Active');

-- Password hash below is a placeholder — replace with a real bcrypt hash
-- generated by the backend before running in any real environment.
INSERT INTO users (employee_id, password_hash)
SELECT id, '$2a$10$REPLACE_WITH_REAL_BCRYPT_HASH'
FROM employees WHERE email = 'admin@assetflow.local';
